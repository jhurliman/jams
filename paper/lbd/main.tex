...
Means and intervals are recomputed from committed per-track score archives;
...
(Table~\ref{tab:comparison}). Paired 95\% bootstrap intervals recomputed from the
committed per-track score archives are [0.3471, 0.3713] and [0.3017, 0.3271], with
YourMT3+ ahead on all 151 pairs in both conditions. The transcriber ordering
persists after separation in this configuration. A stronger
...
the paired F1 difference is $-0.0104$ [$-0.0209$, $+0.0004$], an interval that
includes zero. Without a controlled mechanism test, this is a diagnostic
...
the per-recording score archives behind this comparison, with checksums and a
checker that rejects missing archives or mismatched track IDs before recomputing
the means and intervals above. For the one arm whose note predictions were
archived (YourMT3+ on reference input), rescoring from the raw notes against
rebuilt Slakh references reproduces every archived per-track value; the other
arms are verified at the stored-score level, and no inference was rerun.
