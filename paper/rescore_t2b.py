...
for mid in test_ids:
    ...
    for stem_id, stem in mt.tracks.items():
        ...
        if audio_present != rendered_flag:
            audio_flag_mismatch.append(f"{mid}/{sid}: remote_flac={audio_present} audio_rendered={rendered_flag}")
    ...
    for cls in ("bass", "other"):
        ...
        info[cls] = {
            "stems": [{"id": sid, "program": s.program_number, "inst_class": s.instrument,
                       "is_drum": s.is_drum} for sid, s in members],
...
checks = {
    ...
    "bass_eligible_eq_archive_bass_ids": sorted(eligible["bass"]) == sorted(
        t for (t, s) in arch_by if s == "bass"),
    "bass_eligible_eq_jsonl_bass_ids": sorted(eligible["bass"]) == sorted(
        t for (t, s) in est_by if s == "bass"),
...
for mid in test_ids:
    for cls in ("other", "bass"):
        ...
        if ref_path is None and est is None and arch is None:
            continue
        row: dict = {"track_id": mid, "stem": cls,
                     "ref_midi": ref_path, "has_est": est is not None,
                     "in_archive": arch is not None}
        if ref_path is None or est is None:
            issues.append(f"{mid}/{cls}: ref={ref_path is not None} est={est is not None} archive={arch is not None}")
            row["archived"] = arch
            results.append(row)
            continue
...
summary["bass_variant_matching_archive"] = [
    v for v, c in summary["bass"].items() if c["max_abs_diff_note_f"] is not None and c["max_abs_diff_note_f"] <= 1e-6]
...
for r in results:
    if "n_ref_notes" in r:
        note_counts[r["track_id"]][r["stem"]] = {"ref": r["n_ref_notes"], "est_raw": r["n_est_notes_raw"]}
...
out = {
    ...
    "inputs": {
        ...
        "mirdata_index": str(ds.index_path), "mirdata_index_fetched_now": index_fetched,
    },
    "scoring": {
        "scorer": scorer_source, "gm_helpers": gm_source,
        ...
        "bass_variants": {"raw": "jsonl notes as-is", "shift12": "gm.shift_bass_notes (+12, cap 127)",
                          "mono": "gm.monophonic_filter", "mono_shift12": "monophonic_filter then shift_bass_notes (production order in stems.py)"},
        "other": "no shift, no filter",
    },
    "versions": {"python": sys.version.split()[0], "mir_eval": mir_eval.__version__,
                 "pretty_midi": pretty_midi.__version__, "numpy": np.__version__,
