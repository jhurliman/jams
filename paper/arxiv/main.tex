...
archives with checksums, and recompute every reported interval from them; the
remaining verification gap is that only one arm's note predictions were archived.
\end{abstract}
...
per-recording score archives (runs T1, T2b, S4, T10, and the HT-Demucs run S1) are
committed under \path{paper/evidence/} with SHA-256 checksums, and
\path{paper/verify_results.py} recomputes the means and paired intervals in this
paper from them, refusing missing archives, duplicate rows, or mismatched track
sets. The tables and figure regenerate from a summary snapshot that records that
verification. The repository is the only hosting dependency; no cloud object store
is required to reproduce the analysis.

What the archives do not contain: the basic-pitch and end-to-end note predictions
were not saved, so those arms are verified at the stored-score level only. The
YourMT3+ reference-input predictions were saved and are rescored against the
grouped references independently of the stored fields; that comparison and the
checkpoint and environment record are documented in the repository
(\path{paper/PROVENANCE.md}). No model inference was rerun for this revision, and no
missing measurement has been estimated or synthesized.
