...
Source code, the experiment ledger, and the per-recording evidence behind the Slakh
comparisons (Sections~3--4) and the structure appendix are in the repository at
\url{https://github.com/jhurliman/jams}; the key, tempo, and drum appendix rows are
ledger summaries whose archived outputs were not recovered. The five
...
verification. Reproducing the analysis needs no private cloud storage: everything
beyond the repository comes from public dataset downloads (the Slakh2100-redux test
MIDI and metadata from Zenodo for the rescore, and the Raveform annotation archive
for the structure recomputation); no audio is required for either.
