...
We specify the scoring and aggregation conventions and distinguish public summary
statistics from per-recording archives that still need to be released for independent
recomputation.
\end{abstract}
...
merges the corresponding MIDI instruments. Slakh contains synthesized instrumental
...
and missing groups must therefore be reported separately. The end-to-end ledger
records 151 completed multitracks and zero pipeline failures, but does not preserve
the bass support count in its summary. We do not infer that count from the number
of mixtures.
...
$n=151$. Reference input is the summed ground-truth group. Brackets are archived
95\% bootstrap intervals; intervals were not available for the end-to-end rows.
T1/T2b/S4/T10 identify runs in the project ledger.}
\label{tab:transcription}
\input{tables/transcription.tex}
\end{table}
\begin{figure}[t]
\centering
...
Only reference-input intervals are available; dots without intervals should not
be read as more precise. Values are drawn from the committed summary snapshot,
...
151 reported pairs. This is the strongest numerical contrast in the existing
summaries, although independent recomputation requires the original per-recording
outputs.

After the same SCNet separator, the scores are 0.4733 and 0.7877, a difference of
$+0.3144$. Relative to each model's reference-input score, separation is associated
with decreases of 0.0164 for basic-pitch and 0.0611 for YourMT3+. The larger
absolute decrease for YourMT3+ does not reverse the model ordering. No paired
interval for these end-to-end contrasts is available in the public record; we
...
the historical ADTOF port for drums, on the same nominal 151-track test split.
The eligible bass count is not archived in these summaries. Differences and
confidence intervals must be recomputed on exact matched IDs before inferential use.}
\label{tab:separation}
\input{tables/separation.tex}
\end{table}
...
from 0.5845 to 0.5741. The opposite directions in these averages are an observed
discrepancy, not a demonstrated population-level reversal: paired uncertainty for
the 0.0104 F1 difference is missing.
...
Source code, the experiment ledger, and reported aggregate results are available
at \url{https://github.com/jhurliman/jams}. This revision audits repository snapshot
\texttt{71e61b6}. The per-recording evaluation archives referenced by that snapshot
are not committed or attached to a public release; the documented project object
store was not accessible for this review. The tables and figure in this revision
regenerate from an explicitly labeled summary snapshot. That is not a reproduction
of model inference or of the archived confidence intervals. Releasing the original
predictions, manifests, exact checkpoint identifiers, and environment records is
necessary for independent recomputation. No missing measurements have been estimated
...
paired difference is $+0.0597$ \ci{+0.0034}{+0.1100}. These scores follow the
project's fixed $+12$-semitone shift of estimated bass notes and a monophonic
post-filter. The service applies the shift in its orchestrator; the evaluation
CLI also offers an additional shift, making exact run arguments material.
...
\ci{0.175}{0.308} on buildup. The code averages the fraction correctly labeled
within each reference segment, then averages within each track and across eligible
tracks. This is not corpus-duration-weighted accuracy or a precision measure;
overprediction can raise coverage.
